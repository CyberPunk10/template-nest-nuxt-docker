...
Проверяется единственный хост-порт — `NGINX_HOST_PORT` из корневого `.env`: наружу публикуется только reverse proxy, остальные сервисы живут во внутренней сети и хост-портов не занимают.
