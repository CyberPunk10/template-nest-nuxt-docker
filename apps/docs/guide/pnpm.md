...
`corepack use pnpm@X` не только переключает версию — он же обновляет `packageManager` в `package.json` и добавляет к ней integrity-хэш (`pnpm@11.12.0+sha512.<hash>`), которым Corepack проверяет целостность скачанного бинарника при каждой установке.
