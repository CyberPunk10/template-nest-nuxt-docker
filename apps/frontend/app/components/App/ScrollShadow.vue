...
export default {
  ...
  props: {
    ...
    withoutIgnoreSwipe: Boolean, // по умолчанию игнорирует события свайва, этот флаг это отменяет
    hideScrollBars: Boolean, // скрыть полосу прокрутки
  },
  emits: [
    'update-shadow', // только когда появляется или исчезает тень
    'scroll-handler',
  ],
  setup(props, { emit }) {
    // refs
    const appScrollShadowRef = ref(null)
    const prevElem = ref(null)
    const nextElem = ref(null)
    ...
    watch(
      () => shadowTop.value,
      (val) => updateShadowTop(val),
    )
    watch(
      () => shadowLeft.value,
      (val) => updateShadowLeft(val),
    )
    watch(
      () => shadowRight.value,
      (val) => updateShadowRight(val),
    )
    watch(
      () => shadowBottom.value,
      (val) => updateShadowBottom(val),
    )
    watch(() => widthDiv.value, throttledScrollHandler)
    watch(() => heightDiv.value, throttledScrollHandler)
    watch(
    ...
    function updateShadowTop(val) {
      ...
      if (!props.ignoreAutoShadowTop && !props.ignoreAutoShadowAll) {
        updateAutoShadow(payload)
      }
    }

    function updateShadowBottom(val) {
      const payload = {
        position: 'bottom',
        process: processY.value,
        val,
      ...
      emit('update-shadow', payload)
      if (!props.ignoreAutoShadowBottom && !props.ignoreAutoShadowAll) {
        updateAutoShadow(payload)
      }
    }

    function updateShadowLeft(val) {
      const payload = {
        position: 'left',
      ...
      emit('update-shadow', payload)
      if (!props.ignoreAutoShadowLeft && !props.ignoreAutoShadowAll) {
        updateAutoShadow(payload)
      }
    }

    function updateShadowRight(val) {
      const payload = {
        position: 'right',
        process: processX.value,
        val,
      ...
      emit('update-shadow', payload)
      if (!props.ignoreAutoShadowRight && !props.ignoreAutoShadowAll) {
        updateAutoShadow(payload)
      }
    }

    function scrollHandler($event, force = false) {
      ...
      const payload = {
        hasHBar: hasHBar.value,
        hasVBar: hasVBar.value,
      }
    ...
    function updateAutoShadow(data) {
      const payload = { ...data }

      if (data.position === 'top') {
        let newPrevElem = appScrollShadowRef.value?.previousElementSibling

        // если нет соседнего элемента, то ищем родителя
        if (!newPrevElem) {
          newPrevElem = appScrollShadowRef.value?.parentElement
        ...
        let newNextElem = appScrollShadowRef.value?.nextElementSibling

        // если нет соседнего элемента, то ищем родителя
        if (!newNextElem) {
          newNextElem = appScrollShadowRef.value?.parentElement
          payload.isParent = true
        }
    ...
    function useScrollWithMouseMovements() {
      ...
      function addMouseEvents() {
        appScrollShadowRef.value.addEventListener('mouseleave', mouseLeaveHandler)
        appScrollShadowRef.value.addEventListener('mousemove', mouseMoveHandler)
        appScrollShadowRef.value.addEventListener('mousedown', mouseDownHandler)
        appScrollShadowRef.value.addEventListener('mouseup', mouseUpHandler)
        appScrollShadowRef.value.addEventListener('dragstart', dragStartHandler)
      }

      function removeMouseEvents() {
        if (!appScrollShadowRef.value) return
        appScrollShadowRef.value.removeEventListener('mouseleave', mouseLeaveHandler)
        appScrollShadowRef.value.removeEventListener('mousemove', mouseMoveHandler)
        appScrollShadowRef.value.removeEventListener('mousedown', mouseDownHandler)
        appScrollShadowRef.value.removeEventListener('mouseup', mouseUpHandler)
        appScrollShadowRef.value.removeEventListener('dragstart', dragStartHandler)
      }
    ...
    return {
      ...
      scrollY,
      shadowBottom,
      shadowTop, // экспортируем чтобы можно было снаружи вызвать (из другого компонента)
